...
201 表示更新了数据 对应 `PUT请求 POST请求`

204 NO  Content 请求成功拿到请求头

206 服务器处理成功 ，拿到请求体的一部分，一般是客户端发起的是（范围请求）或者文件的断点续传

**3xx 重定向相关**

301 永久重定向，`说明请求的资源不存在 请求的URI 重定向到其他位置了`。 重定向请求通常（由于历史原因会把POST请求）使用GET方法，而不管请求用什么方法。

302 临时重定向，请求资源还在，但需要使用另一个URL使用，重定向请求通常会使用GET方法，而不管请求用什么方法。

303 临时重定向，重定向请求必须使用GET方法。

304 不具有跳转的含义，表示资源未修改，重定向已缓存的文件， 缓存重定向。

307 临时重定向，重定向请求必须使用与原请求相同的方法和包体发起请求。

308 （HTTP/1.1）永久重定向的请求必须使用与原请求相同的方法和包体发起请求。

**4xx 客户端的报文有误**

400 客户端请求的报文错误（一个笼统的错误，没有明确指明是什么错误）

401 表示用户没有权限请求该页面

403 表示服务器禁止请求该内容，不是客户端出错。

404 无法找到该内容，有可能客户端的URL错误
...
502 该扮演网关或代理角色的服务器从上游服务器中接收到的响应是无效的。注意，502 错误通常不是客户端能够修复的，而是需要由途经的服务器或者代理服务器对其进行修复
...
GET 请求获取服务器资源是安全的 不会修改服务器的内容.
POST 请求 是不安全的,会修改服务器上的内容.
